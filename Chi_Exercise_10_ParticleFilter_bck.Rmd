---
title: "Activity 10 - Particle Filter"
...
SSEM.orig <- function(X,params,inputs,timestep=1800){ 
  ...
  #Rh[which(is.infinite(Rh)&Rh<0)]=0 
  ## turnover
...
## why here is a factor of 3? should it be 44/12~=3.67? 
...
```{r}
### resampling particle filter - with parameter uncertainty
#params2=params  # save for later use
sample=0
...
save(output,output.ensemble,LAIlike,hist.params,inputs,file="Ex10.output_bck.RData") 
...
plot(Mtime[Msel],LAIm.ci[2,],ylim=range(c(range(LAIm.ci),range(LAIr,na.rm=TRUE))),
     type='n',ylab="LAI",xlab="Time",main="resampling PF with parameter uncertainty")
ciEnvelope(Mtime[Msel],LAIm.ci[1,],LAIm.ci[3,],col=col.alpha("lightGrey",0.5))
ciEnvelope(Mtime[Msel],LAIpf[1,],LAIpf[3,],col=col.alpha("lightBlue",0.5))
...
## reimplimentation of the rdirichlet function from MCMCpack
## to fix bug in how it handles alpha as a matrix
rdirichlet.orig2 = function (n, alpha) 
{
  l <- length(alpha)
  if(is.matrix(alpha)) l <- ncol(alpha) ## fix
  x <- alpha#, byrow = TRUE)
...
rdirichlet2 <- cmpfun(rdirichlet.orig2)  ## byte 
...
alloc = matrix(c(Ra,(1-0.315)*Ra,0.315*Ra),1) # prior mean on allocation
Neff = matrix(100,ne) 
params$falloc = rdirichlet2(ne,Neff%*%alloc)  ## Ra, wood, leaf

params$SLA = rep(mean(SLA),ne)
params$alpha = rep(0.02,ne) ## light use efficiency
params$Q10 = rep(2.1,ne)  ## soil respiration Q10
params$Rbasal =rep(0.2,ne)/(params$Q10^2.5) #umol/m2/sec per Mg/ha of SOM

## use moment matching to convert litter and CWD turnover mean & SD to beta dist'n
lit=mean(litter)/mean(Bleaf)
lit.mu = mean(lit)*timestep/86400/365 
CWD.mu = 1/142*timestep/86400/365
params$litter= rep(lit.mu,ne)
params$CWD = rep(CWD.mu,ne) 

## Process error
params$tau.leaf = rep(1/100,ne)
params$tau.stem = rep(1/10,ne)
params$tau.soil =rep(1/10,ne)  
...
for(t in 1:nt){
  ...
  if(t%%(48*8) == 0){ ## if remainder == 0
    ...
    if(!is.na(LAIr[sample])){  ## if observation is present
      ...
      ## resample 
      index = sample.int(ne,ne,replace=TRUE,prob=wt)
      X = X[index,]  #To make the comparison unbiased, keep the same index as previous resampling that is PF with parameter uncertainty.
      params = update.params(params,index)    
    }
    hist.params[[sample+1]] = params
  }
...
plot(Mtime[Msel],LAIm.ci[2,],ylim=range(c(range(LAIm.ci),range(LAIr,na.rm=TRUE))),
     type='n',ylab="LAI",xlab="Time",main="resampling PF without parameter uncertainty")
ciEnvelope(Mtime[Msel],LAIm.ci[1,],LAIm.ci[3,],col=col.alpha("lightGrey",0.5))
ciEnvelope(Mtime[Msel],LAIpf[1,],LAIpf[3,],col=col.alpha("lightBlue",0.5))
ciEnvelope(Mtime[Msel],LAIpr.ci[1,],LAIpr.ci[3,],col=col.alpha("lightGreen",0.5))
points(Mtime,LAIr)    
