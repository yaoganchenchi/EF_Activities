...
The objective of today's exercise is to provide a quick introduction to some common tools for dealing with big data. 
**It seems that sometimes MODISTools has some problem (It worked in the ealier of this week. But on Feb 14, it did not work). **  
...
```{r}
FIA = read.csv("http://apps.fs.fed.us/fiadb-downloads/RI_SEEDLING.CSV",skip=0)
y=tapply(FIA$TREECOUNT,as.factor(FIA$SPCD),sum,na.rm=TRUE)
y2=sort(y, decreasing = TRUE)
y3=log(y2)
plot(y3,xlab='rank',ylab='log(abundance)')
...
```{r}
FLUXNET = getURL("http://fluxnet.ornl.gov/site_status")
FLUXNET_table = readHTMLTable(FLUXNET)[[1]]
FLUXNET_ID = FLUXNET_table$`FLUXNET ID`
country=substr(FLUXNET_ID, start=1, stop=2)
number=data.frame(sort(tapply(rep(1,length(country)),as.factor(country),sum),decreasing = TRUE)) # a table count the numebr of flux towers in each country
...
```{r}
myCode = readLines("Chi_Exercise_03_BigData.Rmd")  ## read unstructured text
...
```{r}
a=nc_open("http://thredds.daac.ornl.gov/thredds/dodsC/ornldaac/1220/mstmip_driver_global_hd_climate_tair_2004_v1.nc4")
print(a) # check the var names and dimensions
# tair matrix: Upper left pixel center [179.75E, 89.75N]
# Boston Lon: [71.5W, 71 W] ~ index of Lon = 218
# Boston Lat: [42.5N, 42N]  ~ index of Lat = 96
tair = ncvar_get(a,"tair",c(218,96,1),c(1,1,-1))
time = ncvar_get(a,"time_bnds")
plot(time[1,]-time[1,1]+1, tair,type = 'l',xlab='doy',ylab='air temperature',main='Boston 2004')
nc_close(a)
```
...
```{r}
par(mfrow=c(2,2))
plot(time,EVI,type='l',main='EVI variation') # plot EVI vs time
for(i in 1:3){
plot(doy,filter(NEE[i,],rep(1/24,24)),type='l',main=paste("Height =",height[i],"m"),ylab='filter(NEE)')
}
```

```
Observed Patterns: 
1. When EVI is high, NEE is low. When EVI is low, NEE is high.
2. Both EVI and NEE has seasonality. In the summer, EVI reaches peak, NEE reaches valley. 

Reason:EVI is generally positively correlated to LAI. When EVI is high, it means that the photosynthesis is very active during that period. Vegetation uptake more CO2 from atmosphere per unit time. When NEE is negateive, it means the atmosphere is losing CO2 (vegetation is sinking CO2). Thus, the absolute value of NEE becomes larger, but its sign is negative.
```
...
**pseudocode/outline**
```
#Let's assume we are using R
#Step 1: This is Ccript one, which will be only one-time running. Assume we already download the full FIA database. Now we are going to make a log file to save the "last updata date" list
fia_html <- getURL("http://apps.fs.fed.us/fiadb-downloads/datamart.html")
fia_table = readHTMLTable(fia_html)[[6]]
update_log_old = as.Date(fia_table[,"Last Modified Date"])  #extract the "last updata date"
update_log_old[is.na(update_log_old)]="1900-01-01"  # set NA to "1900-01-01"
write.csv(update_log_old,file = 'update_log_old.csv',row.names=F) #export the "last updata date" list
```

```
#Step 2: Script two. Check update at 00hh00mm on the 7th day of each week, use MAILTO to activate this part
update_log_old=as.matrix(read.csv('update_log_old.csv',)) # load the previous "last updata date"

#get the new "last updata date" list
fia_html <- getURL("http://apps.fs.fed.us/fiadb-downloads/datamart.html")
fia_table = readHTMLTable(fia_html)[[6]]
update_log_new = as.Date(fia_table[,"Last Modified Date"]) #extract the current "last updata date" from the web
update_log_new[is.na(update_log_new)]="1900-01-01" 
update_log_new_to_save = update_log_new   # copy the new "last updata date" list. Prepare to save after downloadling
update_log_new = as.matrix(as.character(update_log_new))

# compare the previous and new "last updata date", return the index 
change_index = update_log_new!=update_log_old

#get the CSV name
change_file_name = as.matrix(fia_table$`CSV Files`[change_index])

#download those updated file using for loop
for (i in length(change_file_name)){
   down_url= paste('http://apps.fs.fed.us/fiadb-downloads/',change_file_name[i],sep = "")
   system(paste('wget ',down_url,sep=""))
}

#export the new "last update date"
write.csv(update_log_new_to_save,file = 'update_log_old.csv',row.names=F) #replace the "last updata date"

```

```
# Step 3
Compile the above two scripts into shell language. For example, integrate them into get_fia.sh.
Or we can just traslate the R code into shell language, such as bash. Since I am not familar with any shell language, I just write the pseudocode using R grammar.
```


**cron table, update at 00hh00mm on the 7th day of each week**
```
MAILTO=chenchi@bu.edu
0 0 * * 7 /home/scratch/ge585_lab/NOMADS/get_fia.sh
```
